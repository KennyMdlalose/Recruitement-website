...
$result = mysqli_query($conn, "SELECT * FROM postJob WHERE division = 'Film&media'");
?>
    ...
    <div class="job-listing">
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                ...
                // Check if the job post is closed
                $job_post_status = ($current_date >= $end_date) ? 'closed' : 'open';
                ...
                // Apply Button: Disabled if the job is closed
                echo '<input type="submit" value="Apply Now" class="btn" '. ($job_post_status == 'closed' ? 'disabled' : '') .'>';
                echo '</form>';
